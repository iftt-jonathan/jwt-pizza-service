...
test('updateUser', async () => {
    const testUserId = await DB.getID(connection, 'email', testUser.email, 'user');
    const user = await DB.updateUser(testUserId, 'newEmail' + testUser.email, testUser.password);
    expect(user).toMatchObject({
        name: testUser.name,
...
test('deleteFranchise', async () => {
    const franchiseId = await DB.getID(connection, 'name', testFranchise.name, 'franchise');
    await DB.deleteFranchise(franchiseId);

    const [rows] = await connection.query('SELECT * FROM franchise WHERE id=?', [franchiseId]);
...
test('deleteStore', async () => {
    ...
    const [rows] = await connection.query('SELECT * FROM store WHERE id=?', [store.id]);
